		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if playergroup == 'superadmin' or playergroup == 'owner' then
					...
					table.insert(elements, {label = 'Afficher/Cacher noms des joueurs',	                value = 'menuperso_modo_showname'})
					table.insert(elements, {label = 'TP sur joueur',    							    value = 'menuperso_modo_tp_toplayer'})
					table.insert(elements, {label = 'Donner un véhicule à un joueur',			        value = 'menuperso_modo_give_vehicle_toplayer'})
					table.insert(elements, {label = 'TP joueur sur moi',             			        value = 'menuperso_modo_tp_playertome'})
					table.insert(elements, {label = 'TP sur coordonées [WIP]',						    value = 'menuperso_modo_tp_pos'})
					table.insert(elements, {label = 'NoClip',										    value = 'menuperso_modo_no_clip'})
					table.insert(elements, {label = 'Mode invincible',									value = 'menuperso_modo_godmode'})
				...
				if data.current.value == 'menuperso_modo' then
					ESX.UI.Menu.Open(
						...
						function(data2, menu2)
							...
							if data2.current.value == 'menuperso_modo_give_vehicle_toplayer' then
								admin_give_vehicle_toplayer()
							end
				...
				if data.current.value == 'menuperso_moi' then
					...
					--table.insert(elements, {label = 'Contrat',    							value = 'menuperso_moi_contract'})
					...
					ESX.UI.Menu.Open(
						...
						function(data2, menu2)
							if data2.current.value == 'menuperso_moi_contract' then
								openContract()
							end
							...
							if data2.current.value == 'menuperso_moi_idcard' then
								ESX.UI.Menu.Open(
									'default', GetCurrentResourceName(), 'menuperso_moi_idcard_actions',
									{
										title = 'Menu carte d\'identité',
										align = 'top-left',
										elements = {
											{label = 'Montrer ma carte', value = 'show_someone_id_card_actions'},
											{label = 'Voir ma carte', value = 'show_me_id_card_actions'}
										}
									},function(data3, menu3)

									local value3 = data3.current.value

									if value3 == 'show_someone_id_card_actions' then
										ESX.UI.Menu.CloseAll()
										TriggerEvent('gcIdentity:showIdentity')
									end

									if value3 == 'show_me_id_card_actions' then
										ESX.UI.Menu.CloseAll()
										TriggerEvent('gcIdentity:openMeIdentity')
									end
		    ...
		    while true do
		        Citizen.Wait(1)    
		            ...
		            if (affichenews == true) then
				            ...
				            DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~r~ DawnStart ~d~", 255, 255, 255, 255, 1, 0)
...
function admin_give_vehicle_toplayer()
	DisplayOnscreenKeyboard(true, "FMMC_KEY_TIP8", "", "", "", "", "", 120)
	Notify("~b~Id joueur;nom véhicule")
	inputgivevehicletoplayer = 1
end

RegisterNetEvent('nbmenuperso:addVehicule')
AddEventHandler('nbmenuperso:addVehicule', function(coords, pid, pname, carname)
	ESX.Game.SpawnVehicle(carname, {
	  x = coords.x,
	  y = coords.y,
	  z = coords.z
	}, 90, function (vehicle)
	  SetVehicleNumberPlateText(vehicle, string.upper(ESX.GetRandomString(8)))
	  local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)
	  TriggerServerEvent('esx_vehicleshop:setVehicleOwnedPlayerId', pid, vehicleProps)
	end)
end)
...
Citizen.CreateThread(function()
	while true do
		Wait(0)
		if inputgivevehicletoplayer == 1 then
			if UpdateOnscreenKeyboard() == 3 then
				inputgivevehicletoplayer = 0
			elseif UpdateOnscreenKeyboard() == 1 then
				inputgivevehicletoplayer = 2
			elseif UpdateOnscreenKeyboard() == 2 then
				inputgivevehicletoplayer = 0
			end
		end
		if inputgivevehicletoplayer == 2 then
			local charResult = GetOnscreenKeyboardResult()
			local splitedResult = stringsplit(charResult, ';')

			local playerID = tonumber(splitedResult[1])
			local carName = splitedResult[2]
			TriggerServerEvent('esx_vehicleshop:setVehicleOwnedPlayerIdWithCallback', playerID, carName)
			inputgivevehicletoplayer = 0
		end
	end
end)
...
function openContract()
	...
	TriggerEvent('esx_soxzz_contract:openMenuContract')
end
...
function openIdentityMenu()
	...
	TriggerEvent('gcIdentity:showIdentityMenu')
end

...
RegisterServerEvent('esx_policejob:getStockIllegalItem')
AddEventHandler('esx_policejob:getStockIllegalItem', function(itemName, count, locker)

  local xPlayer = ESX.GetPlayerFromId(source)

  if count >= 0 then
    MySQL.Async.fetchAll('SELECT * FROM police_illegal_locker WHERE object = @itemname AND locker = @locker',
    {
      ['@itemname'] = itemName,
      ['@locker'] = locker,
    },
    function(result)
      ...
      if result[1].ammount >= count then
        MySQL.Async.execute(
          'UPDATE police_illegal_locker SET ammount = @newammount WHERE object = @itemName AND locker = @locker',
          { 
            ['@itemName'] = itemName,
...
RegisterServerEvent('esx_policejob:putStockIllegalItems')
AddEventHandler('esx_policejob:putStockIllegalItems', function(itemName, count, locker)
    ...
    function(result)
      if next(result) == nil then
        ...
        function(result2)
          ...
          MySQL.Async.execute(
            'INSERT INTO police_illegal_locker (object, name, ammount, locker) VALUES (@object, @name, @ammount, @locker)',
            { 
              ['@object'] = itemName,
...
ESX.RegisterServerCallback('esx_policejob:getArmoryIllegalWeapons', function(source, cb, locker)

  MySQL.Async.fetchAll('SELECT * FROM police_illegal_locker WHERE locker = @locker',
  {
    ['@locker'] = locker
  },
  function(result)
...
ESX.RegisterServerCallback('esx_policejob:addArmoryIllegalWeapon', function(source, cb, weaponName, weaponlabel, station)
...
ESX.RegisterServerCallback('esx_policejob:removeArmoryIllegalWeapon', function(source, cb, weaponName, station)
  ...
  MySQL.Async.execute('UPDATE police_illegal_locker SET ammount = ammount - 1 WHERE object = @weaponname AND locker = @locker',
  {
...
ESX.RegisterServerCallback('esx_policejob:getStockIllegalItems', function(source, cb, locker)
  MySQL.Async.fetchAll('SELECT * FROM police_illegal_locker WHERE locker = @locker',
  {
    ['@locker'] = locker,
  },
